...
import moCreatures.proxies.EntityItemProxy;
import moCreatures.proxies.EntityWaterMobProxy;
...
public class EntityCustomWM extends EntityWaterMobProxy
{
	...
	protected void updatePlayerActionState()
	{
		...
		if(playerToAttack == null)
		{
			...
			if(playerToAttack != null && playerToAttackInWater())
				pathToEntity = worldObj.getPathToEntity(this, playerToAttack, f);
		} else if(!playerToAttack.isEntityAlive() || !playerToAttackInWater())
			playerToAttack = null;
		else
		{
		...
		if(!hasAttacked && playerToAttack != null && playerToAttackInWater() && (pathToEntity == null || rand.nextInt(20) == 0))
			pathToEntity = worldObj.getPathToEntity(this, playerToAttack, f);
		...
		if(vec3d != null)
		{
			...
			if(d3 > 0.0D && playerToAttack != null && playerToAttackInWater())
				isJumping = true;
		}
		if(playerToAttack != null)
	...
	public EntityItem getClosestFish(Entity entity, double d)
	{
		...
		for(int i = 0; i < list.size(); i++)
		{
			...
			if(entityitem1.item.itemID != Item.fishRaw.shiftedIndex || !((EntityItemProxy)entityitem1).inWater())
				continue;
