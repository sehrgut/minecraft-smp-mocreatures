...
//            ServerCommand, EntityPlayerMP, WorldServer, Item, 
//            ServerConfigurationManager, ItemStack, NetServerHandler, Packet3Chat, 
//            PropertyManager, ICommandListener
        ...
        if(s.toLowerCase().startsWith("give "))
        {
            ...
            if(entityplayermp4 != null)
            {
                try
                {
                    ...
                    {
                        sendNoticeToOps(s1, (new StringBuilder()).append("Giving ").append(entityplayermp4.username).append(" some ").append(k).toString());
                        int l = 1;
                        if(as1.length > 3)
                        {
                            l = tryParse(as1[3], 1);
        ...
        if(s.toLowerCase().startsWith("time "))
        {
            ...
            try
            {
                ...
                if("add".equalsIgnoreCase(s10))
                {
                    ...
                    sendNoticeToOps(s1, (new StringBuilder()).append("Added ").append(j).append(" to time").toString());
                } else
                if("set".equalsIgnoreCase(s10))
                {
                    worldserver.setWorldTime(j);
                    sendNoticeToOps(s1, (new StringBuilder()).append("Set time to ").append(j).toString());
                } else
                {
